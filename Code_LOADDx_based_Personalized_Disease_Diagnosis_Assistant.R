...
# gene start index for the Gene_expression_data_full dataframe
s_index <- 10
...
for(p in seq(P_start,P_end,P_step)){  # loop of p is for top genes 
  ...
  for(q in seq(Q_start,Q_end,Q_step)){  # loop of q is for bottom genes 
    ...
    for(l in 1:s){ # loop of l for number of subjects
      ...
      # code for assigning reward based on top P genes (log-odds based)
      for(j in 1:p){ # loop of j for number of genes
        Disease_IDs <- KG_ctd_gene_dis_required_genes[KG_ctd_gene_dis_required_genes$GeneSymbol == names(Gene_Transition_Matrix_top_p_Genes)[j], "Disease_ID" ]
      ...
      # code for assigning penalty based on bottom Q genes (log-odds based)
      ...
      for(j in 1:q){ # loop of j for number of bottom genes
        Disease_IDs <- KG_ctd_gene_dis_required_genes[KG_ctd_gene_dis_required_genes$GeneSymbol == names(Gene_Transition_Matrix_bottom_q_Genes)[j], "Disease_ID" ]
      ...
      # calculate Log-odds for top p and bottom q genes
      for(m in 1:dim(Data_Unique_Disease)[1]){  # loop of m for number of disease
        Data_Unique_Disease$Disease_Weight[m] <- log((Data_Unique_Disease$Disease_Weight[m] + 1)/ (p+1-Data_Unique_Disease$Disease_Weight[m]))
        Odds_bottom_gene$Disease_Weight[m] <- log((Odds_bottom_gene$Disease_Weight[m] + 1)/ (q+1-Odds_bottom_gene$Disease_Weight[m]))
        Data_Unique_Disease$Disease_Weight[m] <- Data_Unique_Disease$Disease_Weight[m] -  Odds_bottom_gene$Disease_Weight[m]
      }
